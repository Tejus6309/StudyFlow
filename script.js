function toggleSidebar() {
    ...
    function deleteTask(id) {
      if (reminderTimeouts[id]) { clearTimeout(reminderTimeouts[id]); delete reminderTimeouts[id]; }
    ...
    function toggleDone(id) {
      ...
      saveTasks();
      // Cancel reminder if task just became done
      const t = tasks.find(t => t.id === id);
      if (t && t.done && reminderTimeouts[id]) {
    ...
    /* Holds the setInterval reference so we can clearInterval it */
    ...
    function loadTimerState() {
      ...
      timerState.isRunning = false;           // never auto-resume on reload
    ...
    /**
     * startTimer()
     * Starts (or resumes) the countdown using setInterval().
    ...
    function startTimer() {
      ...
      timerInterval = setInterval(function () {
        ...
        if (timerState.timeLeft <= 0) {
          // Time's up — stop interval, handle mode switch
     ...
     * Shows a toast, increments sessions if a focus block just ended,
    ...
    function handlePomoSettingsSubmit(e) {
      ...
      if (timerState.isRunning) {
        // If it's running, ensure timeLeft doesn't exceed new maximum
        timerState.timeLeft = Math.min(timerState.timeLeft, newTotalDuration);
      } else {
        // If paused/stopped and at full time, adjust entirely.
        // Or just always reset if it was at old max.
        // simpler: reset to new duration if we aren't mid-session
    ...
    // In-memory map of { taskId -> timeoutId } so we can cancel/replace
    ...
    function scheduleReminder(task) {
      ...
      // Cap at ~24.8 days (max safe setTimeout)
      const MAX_MS = 2_147_483_647;
      if (msUntilReminder > MAX_MS) return;

      reminderTimeouts[task.id] = setTimeout(() => {
        delete reminderTimeouts[task.id];
        // Only fire if task is still pending
        const live = tasks.find(t => t.id === task.id);
        if (!live || live.done) return;
        const timeStr = formatDueTime(live.dueTime);
        sendNotification(
          '⏰ Reminder: ' + live.title,
          'Due in ' + formatReminderLabel(live.reminderMins) + ' at ' + timeStr + '. Don\'t forget!',
          '⏰'
        );
        showToast('⏰ Reminder: ' + live.title);
      }, msUntilReminder);
    }
